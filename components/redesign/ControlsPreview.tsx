...
export function ControlsPreview() {
  ...
  return <div className="mx-auto w-full max-w-[1080px] px-5 md:px-8 space-y-5">
    ...
    <ToolStep n={1} title="Ações e feedback" hint="Estados normal, secundário, indisponível e em andamento.">
      <div className="flex flex-wrap gap-3"><ToolAction loading={loading} onClick={tryAction}>Experimentar botão</ToolAction><ToolAction variant="secondary" onClick={()=>setMessage('Ação secundária acionada.')}>Ação secundária</ToolAction><ToolAction disabled>Indisponível</ToolAction></div>
    ...
    <ToolStep n={2} title="Escolhas e ajustes" icon={<IconStepFormat size={18} />}>
      ...
      <div className="mt-5"><ToolSlider label="Qualidade" min={10} max={100} value={quality} onChange={setQuality} display={v=>v+'%'} /></div>
    </ToolStep>
