 ...
 * o trecho relevante da ferramenta, com marcadores numerados (Mark) apontando
 * onde clicar. Tudo é div+CSS: zero imagem, zero peso de rede.
 */

/* Janela fake de app: header com 3 dots + label, corpo relativo pros Marks. */
export function Shot({
  ...
  return (
    <div className="dark-island relative overflow-hidden rounded-[14px] border border-line-strong bg-[#0b0b0f] shadow-depth-1">
...
/* Marcador numerado com ping — posicionado em % dentro do Shot. */
export function Mark({ n, x, y }: { n: number; x: string; y: string }) {
  return (
    <span className="guide-mark" style={{ left: x, top: y }} aria-hidden>
      {n}
    </span>
  );
}
...
export function MBtn({
  ...
  return (
    <span
      className={
        'inline-flex items-center gap-1.5 whitespace-nowrap rounded-[10px] border px-3 py-1.5 text-[11px] font-bold ' +
        tones[tone]
      }
...
/* Dropzone tracejada */
export function MDrop({ label, sub }: { label: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-1 rounded-[12px] border border-dashed border-violet/40 bg-violet/[0.05] px-4 py-5 text-center">
...
export function MField({
  ...
  return (
    <div className={'flex flex-col gap-1 ' + (grow ? 'flex-1 min-w-[120px]' : '')}>
      ...
      <span className="truncate rounded-[9px] border border-white/10 bg-black/40 px-2.5 py-1.5 text-[11px] text-white/65">
        {value}
      </span>
    </div>
  );
...
/* Slider fake */
export function MSlider({ label, pct, val }: { label: string; pct: number; val: string }) {
  return (
...
/* Toggle fake */
export function MToggle({ on, label }: { on: boolean; label: string }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-[10px] border border-white/10 bg-white/[0.03] px-2.5 py-1.5">
      <span
        className={
...
export function MChip({
  ...
  return (
    <span
      className={
        'inline-flex items-center rounded-full border px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-[0.1em] ' +
        tones[tone]
      }
    >
      {children}
...
export function MQueueItem({
  ...
  return (
    <div className="rounded-[11px] border border-white/10 bg-black/30 px-3 py-2">
