...
function imagePosterUrl(asset: FlowAsset, assets: FlowAsset[], index: number): string | undefined {
  const poster = mediaUrl(asset.posterUrl);
  const media = mediaUrl(asset.url);
  if (poster && poster !== media) {
    try {
      const pathname = new URL(poster).pathname;
      if (!/\.(?:mp4|mov|webm)(?:$|\?)/i.test(pathname) && !pathname.includes('/video/')) return poster;
    } catch { /* mediaUrl already validated the URL; fall through to a generated frame. */ }
  }
  for (let cursor = index - 1; cursor >= 0; cursor -= 1) {
    const candidate = assets[cursor];
    if (candidate.kind === 'image') {
      const image = assetUrl(candidate);
