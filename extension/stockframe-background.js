...
(function () {
  ...
  const allowedActions = new Set(['status', 'configure', 'disconnect', 'list', 'download']);
  ...
  function assertPaidAccount(payload) {
    ...
    const limitRaw = source?.downloads_limit ?? source?.downloadsLimit ?? source?.daily_limit ?? source?.dailyLimit ?? source?.downloads?.limit;
  ...
  async function downloadableResponse(videoId, context) {
    const result = await apiFetch(`/videos/${encodeURIComponent(videoId)}/download`, {
      accept: 'video/*,application/octet-stream,application/json', isDownload: true, context,
    }, async (response) => {
    ...
    return timedFetch(url.toString(), { method: 'GET', cache: 'no-store', redirect: 'follow' }, async (response) => {
  ...
  async function download(tabId, requestId, videoId) {
    ...
    const { bytes, mimeType, filename } = await downloadableResponse(videoId, { tabId, requestId });
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    emit(tabId, requestId, 'SF_ACK', {}).then(async () => {
      ...
      if (message.action === 'download') return await download(tabId, requestId, message.payload?.videoId);
